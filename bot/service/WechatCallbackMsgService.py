...
class WechatCallbackMsgService:
    ...
    async def handle_wechat_message(self, response_data):
        pushType = response_data["pushType"]
        await self.handleOriginMsg(pushType, response_data)

    async def handleSyncMessage(self, response_data):
        # 1 同步消息
        # 消息体
        ...
        await self.handleByMsgType(type, wechatId, response_content_body)
        log.warning(f"[handlePushMsg]sync message not from mobile: {response_data}")

    async def handle_text_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
        ...
        if "@chatroom" in fromWechatId:
            await self.WechatMsgHandleHandle.handle_group_message(wechatId, msgId, fromWechatId, msgContent, msgXml,
        ...
        await self.WechatMsgHandleHandle.handle_user_message(wechatId, msgId, fromWechatId, msgContent, msgXml,
                                                             response_content_body)

        log.info(f"[handle_text_message]text message: {response_content_body}")
        pass

    async def handle_image_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                   msgTime, adminMsgFlag, response_content_body):
        # 图片消息	3
    ...
    async def handle_friend_verify_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                           msgTime, adminMsgFlag, response_content_body):
        # 好友验证消息	37
    ...
    async def handle_friend_recommend_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                              msgTime, adminMsgFlag, response_content_body):
        # 好友推荐消息	42
        log.info(f"[handle_friend_recommend_message]friend recommend message: {response_content_body}")
        pass

    async def handle_chat_emoji_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                        msgTime, adminMsgFlag, response_content_body):
        # 聊天表情	47
        log.info(f"[handle_chat_emoji_message]chat emoji message: {response_content_body}")
        pass

    async def handle_location_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                      msgTime, adminMsgFlag, response_content_body):
        # 位置消息	48
    ...
    async def handle_video_call_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                        msgTime, adminMsgFlag, response_content_body):
        # 音视频通话	50
        log.info(f"[handle_video_call_message]video call message: {response_content_body}")
        pass

    async def handle_mobile_operation_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                              msgTime, adminMsgFlag, response_content_body):
        # 手机端操作消息	51
        log.info(f"[handle_mobile_operation_message]mobile operation message: {response_content_body}")
        pass

    async def handle_system_notification_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                                 msgTime, adminMsgFlag, response_content_body):
        # 系统通知	10000
        log.info(f"[handle_system_notification_message]system notification message: {response_content_body}")
        pass

    async def handle_recall_message(self, wechatId, msgId, fromWechatId, msgContent, msgXml,
                                    msgTime, adminMsgFlag, response_content_body):
        # 撤回消息	10002
    ...
    async def handleLogoutEvent(self, response_data):
        # 3 退出事件
        log.warning(f"[handlePushMsg]logout event: {response_data}")
        pass

    async def handleSendMessageHook(self, response_data):
        # 4 主动发送消息HOOK
        log.warning(f"[handlePushMsg]send message hook: {response_data}")
        pass

    async def handleInterfaceAsyncResponse(self, response_data):
        # 0 接口异步响应
        log.warning(f"[handlePushMsg]async response: {response_data}")
        pass

    async def handleOtherMes(self, response_data):
        # 其他消息类型
        log.warning(f"[handlePushMsg]Unknown pushType: {response_data['pushType']},data:{response_data}")
        pass

    async def handleOriginMsg(self, case, response_data):
        await self.switPushType.get(case, self.handleOtherMes)(response_data)

    async def handleByMsgType(self, case, wechatId, response_content_body):
